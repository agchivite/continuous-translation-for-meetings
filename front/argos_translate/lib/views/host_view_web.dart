...
import '../services/AudioReciever.dart';
import '../services/room_service.dart';
import '../services/speech_to_text_service.dart';
import '../services/text_to_speech_service.dart';
import '../services/translation_service.dart';

class HostViewWeb extends StatefulWidget {
  const HostViewWeb({super.key});

  @override
  HostViewWebState createState() => HostViewWebState();
}

class HostViewWebState extends State<HostViewWeb> {
  ...
  Future<void> _generateRoom() async {
    try {
      ...
      // Aquí, puedes enviar el roomCode al cliente JavaScript
      // Por ejemplo, usando un método de JavaScript para pasar el roomCode
      final js = """
            if (typeof onRoomGenerated === 'function') {
                onRoomGenerated('$roomCode');
            }
        """;
  ...
  void _connectToRoom(String roomId) {
    ...
    // Navegar a una nueva página para recibir audio
    Navigator.push(
      context,
      MaterialPageRoute(
        builder: (context) => AudioReceiver(channel: _channel!),
      ),
    );
  }
